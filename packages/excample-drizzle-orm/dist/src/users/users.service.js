...
let UsersService = class UsersService {
    ...
    async create(createUserDto) {
        ...
        await this.drizzleProd.insert(schema.users).values(createUserDto);
        const user = this.drizzleProd.query.users.findFirst({
            where: (0, drizzle_orm_1.eq)(schema.users.email, createUserDto.email),
        });
        return user;
    }
    findAll() {
